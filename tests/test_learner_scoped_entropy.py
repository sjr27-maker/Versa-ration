"""Replan's entropy (and, by the same fix, HypothesisGenerator's
branch budget) must reflect only the current session's own learner —
HypothesisStore.list_all() has no learner filter at all, so before this
fix a different learner's hypotheses in the same database would
silently inflate or deflate a session's reasoning budget. Verified here
