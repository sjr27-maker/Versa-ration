...
(learner_id, concept_id) rather than recording history. This is a
deliberate difference in contract, not an oversight: `OverlayEntry` is
not a `Hypothesis` and must not be routed through `HypothesisStore`.
"""
...
class LearnerOverlay:
        ...
        async with self._pool.acquire() as conn:
            row = await conn.fetchrow(
                ...
                INSERT INTO learner_overlay (
                    learner_id, concept_id, state, confidence, updated_at
                ) VALUES ($1, $2, $3, $4, NOW())
                ON CONFLICT (learner_id, concept_id) DO UPDATE
                    SET state      = EXCLUDED.state,
                        confidence = EXCLUDED.confidence,
                        updated_at = EXCLUDED.updated_at
                RETURNING concept_id, state, confidence, updated_at
                """,
                learner_id,
                concept_id,
                state.value,
    ...
    async def get_state(
        self, learner_id: UUID, concept_id: str
    ) -> OverlayEntry | None:
        async with self._pool.acquire() as conn:
            row = await conn.fetchrow(
                """
                SELECT concept_id, state, confidence, updated_at
    ...
    async def get_full_overlay(
        ...
        return {row["concept_id"]: self._row_to_entry(row) for row in rows}
